...
4. 食材描述规则：
   - detail必须列出制作该菜品所需的所有原始食材
   - 对于粥类，应该写明具体的谷物，如："小米"而不是"小米粥"
   - 对于汤类，应该写明具体的食材，如："海带"、"排骨"而不是"海带汤"
   - 对于面食，应该写明具体的面粉类型，如："面粉"而不是"面条"

正确示例：
✓ "name": "海带排骨汤", "detail": ["海带", "排骨"]
✓ "name": "小米粥", "detail": ["小米"]
✓ "name": "皮蛋瘦肉粥", "detail": ["大米", "皮蛋", "瘦肉"]

错误示例：
❌ "name": "海带汤", "detail": ["海带汤"]  // 错误：使用成品名作为食材
❌ "name": "小米粥", "detail": ["小米粥"]  // 错误：使用成品名作为食材
❌ "name": "面条", "detail": ["面条"]  // 错误：使用成品名作为食材
...
{{
  ...
  "menu": {{
    ...
    "dishes": [
      {{
        "detail": ["食材1", "食材2", "..."],   //明确菜品所需要使用的食材（菜场标签名），禁止直接使用菜品名称
        "name": "菜品名称",
...
{{
  ...
  "food_details": [
    {{
      ...
      "foodDetail": ["食材1", "食材2", "..."],  //明确菜品所需要使用的食材（菜场标签名），禁止直接使用菜品名称
      "foodName": "替换后的食物名称",
