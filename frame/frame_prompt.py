...
请确保JSON格式正确并只返回{{}}内的正文内容，不要包含任何额外的文本或解释。
"""
...
请遵循以下步骤：
1. 根据分析结果和用户信息，为早餐、午餐和晚餐设计适当的膳食。
2. 主要从当天对应的食材中选择食材，确保每餐符合中国居民的传统饮食习惯：
   - 早餐：以清淡为主，可包括粥、包子、馒头、鸡蛋、豆浆牛奶等早餐。
   - 午餐和晚餐：以中式菜肴为主，通常包括主食（米饭或面食）、一荤一素或两菜一汤等。
3. 为每个菜品提供简短的营养说明，包括其主要营养成分和健康益处。
4. 计算并提供每餐的总热量。
...
请确保JSON格式正确并只返回{{}}内的正文内容，不要包含任何额外的文本或解释。每餐的菜品应符合中国传统饮食习惯，同时考虑用户的健康需求。营养说明应简洁明了，突出食材的针对用户定制的营养成分和健康益处。
"""
